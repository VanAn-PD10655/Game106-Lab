...
namespace ServerGame106.Models
{
    ...
    public class APIGameController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateUserInformation([FromFrom] UserInformationDTO userInformationDTO)
        {
            try
            {
                ...
                user.RegionId = userInformationDTO.RigionId;

                if (userInformationDTO.Avatar != null)
                {
                    var fileExtension = Path.GetExtension(userInformationDTO.Avtar.FileName);
                    var fileName = $"{userInformationDTO.UserId}{fileExtension}";
                    var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwroot/uploads/avatars", fileName);

                    Directory.CreateDirectory(Path.GetDirectoryName(filePath));

                    using (var stream = new FileStream(filePath, FileMode.Create))
                    {
                        await userInformationDTO.Avatar.CopyAsync(stream);
                    }
        ...
        public async Task<IActionResult> ForgotPassword(string Email)
        {
            try
            {
                ...
                _response.IsSuccess = true;
                _response.Notification = "Gửi OTP thành công";
        ...
        public async Task<IActionResult> CheckOTP(CheckOTPDTO checkOTPDTO)
        {
            try
            {
                ...
                var stringOTP = Convert.ToInt32(checkOTPDTO).ToString();
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordDTO resetPasswordDTO)
        {
            try
            {
                ...
                if (user.OTP == stringOTP)
                {
                    ...
                    {
                        _response.IsSuccess = false;
                        _response.Notification = "Đổi mật khẩu không thành công";
                        _response.Data = result.Errors;
                        return BadRequest(_response);
                    }
                }
